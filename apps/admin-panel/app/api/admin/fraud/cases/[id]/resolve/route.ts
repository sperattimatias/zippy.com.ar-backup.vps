...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${base}/api/admin/fraud/cases/${params.id}/resolve`, { method: 'POST', headers: { Authorization: `Bearer ${access}`, 'content-type': 'application/json' }, body: JSON.stringify(body) });
