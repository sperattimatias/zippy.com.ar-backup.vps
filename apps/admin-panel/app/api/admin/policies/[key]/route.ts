...
export async function PUT(req: Request, { params }: { params: { key: string } }) {
  ...
  const res = await fetch(`${base}/api/admin/policies/${params.key}`, { method: 'PUT', headers: { Authorization: `Bearer ${access}`, 'content-type': 'application/json' }, body: JSON.stringify(body) });
